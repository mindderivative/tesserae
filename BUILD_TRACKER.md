...
- `video` has no faithful declarative port available today — tracked as [`tre` issue #2](https://github.com/mindderivative/tre/issues/2) (M25); `time_picker_dial`/`radio_button`/`switch`/`link` are the same "no declarative `NodeKindSpec`" story (M23/M24).
