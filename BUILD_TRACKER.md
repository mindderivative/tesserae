...
**Just closed:** M25 — re-scoped M24's "confirmed blocked" categorization for `video`/`node_graph`/`graph_node` against direct source, since it turned out to be wrong for two of the three. Real correction: `add_node_graph` (`window_factory.rs`) is a plain themed `NodeKind::Rect` (`background: surface_container_low`, `corner_radius`/`elevation` both real fixed `0.0`) — zero new primitives needed, shipped today as `NodeGraph_Component.yaml`. `add_video` is a `NodeKind::Image` seeded with a synthetic transparent 1×1 placeholder — `kind: Image` is already declarative, but `ImageSpec.src` is a required file path with no "blank placeholder" concept, so a faithful port isn't possible without a small `tre`-side change. Rather than ship a non-faithful poster-image approximation, filed [`tre` issue #2](https://github.com/mindderivative/tre/issues/2) requesting `ImageSpec.src` become optional (defaulting to the same synthetic placeholder `add_video` already builds), explicitly scoped to just the placeholder shape — real frame playback (`push_frame`) stays imperative, not requested as a declarative concept. `graph_node` is reclassified: not blocked by a missing `NodeKindSpec` at all, but structurally unfragmentable — `add_graph_node` requires a live `graph` node reference as its own attachment parent, which no `{{ }}`-substitution macro can express; moved into the same "runtime-constructed, not statically declarable" bucket as `tabs`/`menu`/etc., just via a different real mechanism. 60 fragments total. 1 new pytest test (`test_fragments_media_graph.py`), full suite 135 passed (134 prior + 1 new), 0 regressions.

**Previously:** M15-M24 — the macro-expansion engine, its wiring, and 7 full/partial widget categories (59 fragments). See their own entries below.

**Up next:** Two real, independent fronts remain, both requiring further scoping/design decisions before implementation: (1) `tre`-side additive primitive support for `radio_button`/`switch`/`link`/`circular_progress`/`linear_progress`/`loading_indicator`/`time_picker_dial` (7 widgets, mirrors the already-proven M74 `Icon` precedent — `radio_button`/`switch`/`link` would reuse existing `WidgetSpec` fields with zero new ones); (2) a macro-layer loop/repeat construct unlocking `tabs`/`navigation_rail`/`navigation_drawer`/`button_group`/`list_`/`menu` (6 widgets) — real, novel design surface (list-shaped param substitution, per-iteration id namespacing) with a permanent limitation regardless of design: anything built this way is fixed at load/hot-reload time, not reactively live like `tesserae.Repeater`. Neither started; user chose to scope but not yet build either.
