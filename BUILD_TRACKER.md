...
**Up next:** M30 — theme arguments on `App`/`App.load()`, scoped 2026-09-25 (see Milestone 30 below), **not started**: Phase 1 needs the user's decision on where a theme lives (app-wide vs. per screen), because `tre` shares the first-shown screen's theme with the window and never switches it. [`tre` issue #8](https://github.com/mindderivative/tre/issues/8) (bound values lost on `set_theme`/`reconcile`; no `View.set_stylesheet`) may change Phase 3's design. Other named, un-scoped candidates: hot reload for `App.register()`ed screens (Tesserae doesn't know their file), and conditional per-item styling for the 5 Rust-internal-state-dependent-coloring widgets (`tabs`/`button_group`/`navigation_rail`/`navigation_drawer`/`menu`).
...
- `App`/`App.load()` has no theme-related API at all (`theme_seed`/`custom_theme`/`dark`/`stylesheet`) — a real, pre-existing gap, newly relevant now that `component:` fragments can reference MD3 color roles/shape tokens through `App.load()`. `load_view` itself takes them all since M29 Phase 4, reading theme/stylesheet files in Tesserae; a themed screen can use `load_view(...)` + `App.register(...)` meanwhile. **Scoped as M30** (not started).
- Hot reload (M29) can revert live data on screen: when an edit touches a node that has a `{{ }}` binding, `tre`'s `reconcile(spec=...)` re-applies that node's static values but not its binding, so it shows the YAML's placeholder text until the bound `Signal` next changes (reproduced 2026-09-25 against `tre` v0.3.2: editing a bound label's `style.width` turns `'bound'` into `'static'`). `View.set_theme` does the same to every bound node at once. A `tre`-side fix is requested as [`tre` issue #8](https://github.com/mindderivative/tre/issues/8) part 1.
...
- Hot reload (`ViewWatcher`) doesn't watch theme or stylesheet files (M29 Phase 4); a theme edit needs an explicit `set_theme` call with a freshly loaded theme. Theme files: **scoped as M30 Phase 3**. Stylesheet files: blocked — `tre` has no way to change a `View`'s stylesheet after construction (no `set_stylesheet`); requested as [`tre` issue #8](https://github.com/mindderivative/tre/issues/8) part 2.
...
**Status: ⬜ Not started — scoped (2026-09-25), awaiting user decisions.** User-directed: "scope adding theme args to App.load()". Closes the long-standing known gap that `App` has no theme API at all. Since M29, `load_view` already accepts every theme argument (`theme_seed`/`dark`/`default_theme`/`custom_theme`/`stylesheet`, reading the files in Tesserae and handing `tre` `*_spec=` dicts), so the plumbing exists — the real work is deciding where a theme *lives* in a multi-screen `App`, which `tre` itself constrains.
...
### Phase 1 — Decide the API Shape (user decisions) ⬜
- Step 1: where a theme lives. **Recommended:** app-wide, on the constructor — `App(..., theme_seed=, dark=, default_theme=, custom_theme=)` (plus their `*_spec=` forms), applied to every screen `load()` builds — because a theme is window-level in `tre` (above), so one app-wide theme is the only choice that keeps declarative and imperative widgets consistent across `show()` switches. Alternatives: per-screen themes on `load()` only, or both with per-screen overrides — each needs the window-theme mismatch documented as a known limitation — ⬜
- Step 2: stylesheets. **Recommended:** per screen, `App.load(..., stylesheet=)` (and `stylesheet_spec=`), since a stylesheet is a per-`View` layer; optionally an app-wide default stylesheet on the constructor that `load()` uses when none is given — ⬜
- Step 3: whether Phase 3 (hot reload of theme files) is in scope for this milestone, or a follow-up — and whether to wait for `tre` issue #8 (bindings re-applied after `set_theme`; `View.set_stylesheet`) before building it — ⬜

### Phase 2 — Implementation ⬜
- Step 1: `App.__init__` accepts the chosen theme arguments; theme files are read once, through `load_theme` (so a `FontFallbackWarning` fires once, not once per screen), and kept as dicts — `tre` still never gets a path — ⬜
- Step 2: `App.load()` builds every screen through `load_view` with the app-wide theme dicts plus the screen's own stylesheet; the path and `*_spec=` forms stay mutually exclusive, as in `load_view` — ⬜
- Step 3: `App.register()` is unchanged (the caller builds the view, so the caller themes it); its docstring says so, and that `load_view(..., **theme)` is how to give a registered screen the app's theme — ⬜
- Step 4: remove `App.load()`'s docstring note that it takes no theme arguments — ⬜

### Phase 3 — Hot Reload for Theme Files (if in scope per Phase 1) ⬜
- Step 1: with `run(hot_reload=True)`, also watch the app-wide theme files; on change, re-read them on the watcher thread and queue, via `call_soon`, one `View.set_theme(...)` per `load()`ed screen, passing the *complete* selection each time (seed, dark, both theme dicts) since `tre` treats omitted arguments as defaults — ⬜
...
- Step 3: stylesheet files — watch and re-apply them too if `tre` issue #8 part 2 (`View.set_stylesheet`) has landed; otherwise leave them unwatched, as recorded in Known gaps — ⬜

### Phase 4 — Tests, Docs, Tracker ⬜
- Step 1: tests — an app-wide theme reaching every `load()`ed screen (observed through `corner_radius` via a theme's `styles:`, as M29 Phase 4's tests do), a per-screen stylesheet, the path/`*_spec=` exclusivity error, the font warning firing once, `register()` unaffected, and (if Phase 3 is in) a theme-file edit re-theming a live app — ⬜
- Step 2: MkDocs, per the standing rule — `api/app.md` (constructor and `load()` signatures), `guide/themes-and-fonts.md` (replace its "`App.load()` doesn't take theme arguments yet" note), `guide/apps-and-screens.md`; `mkdocs build --strict` clean, new anchors checked — ⬜
