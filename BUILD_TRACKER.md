...
| M32 — Migrate to `tre` 0.3.3 | `████░░░░░░` 40% | 🚧 In progress — Phase 2 of 5 done (2026-09-25) |

**Just closed:** M32 Phase 2 (2026-09-25) — `.venv` pinned to the `tre` v0.3.3 release (a wheel built offline from the tag, replacing the source-tree link); `tre`'s migration script run and reviewed (it wrongly rewrote three Rect `background`s inside `.py` strings — fixed); dict specs, `Switch`'s `is_on` → `selected`, and the docs migrated by hand. 81 → 13 failures on 0.3.3, all Python-API renames for Phase 3. Before that, M30 (2026-09-25) — theme arguments on `App`: an app-wide theme on `App(...)` (`theme_seed`/`dark`/`default_theme`/`custom_theme`, plus `*_spec=` forms), a default stylesheet on `App(...)` that `App.load(..., stylesheet=)` overrides per screen, and `App.build_view()` for `register()`ed screens. Theme is app-wide because `tre` shares the first screen's theme with the window. Theme-file hot reload was split out into M31. 221 → 233 tests, verified against `tre` v0.3.2 built from source (this repo's `.venv` currently loads `tre`'s unreleased 0.3.3).
...
**Up next:** M32 Phase 3 — the Python API renames (`tesserae.widgets`' `switch`/`divider`/`link`/`dialog`/`toolbar`/`slider`, the `foreground=` translations, and the tests' property names), which account for all 13 remaining failures; waiting on the user's go-ahead.
...
**Status: 🚧 In progress — Phase 2 of 5 done (2026-09-25).** User-directed: "scope the 0.3.3 migration as M32"; decisions: "go with your recommendations for M32". `tre` v0.3.3 is released (tag on `bc5e9b6` on `tre`'s `main`, 24 release assets; verified), carrying M90's schema and API renames (full table in `tre`'s `docs/migrating-0.3.3.md`), M91's fix for `tre` issue #8 plus `View.set_stylesheet`, and M92's animatable icon color. Unblocks M31.
...
- Step 1: `tesserae.widgets` delegates per Phase 1 Step 1 — drop the `foreground=`→`color=` translations, and update `switch`/`divider`/`link`/`dialog`/`toolbar` — ⬜
- Step 2: the rest of `src/` — anything that builds specs or reads properties with old names — ⬜
- Step 3: the tests' Python calls and property names (`"thumb_position"`→`"value"`, `set_on`/`get_on`→`set_selected`/`get_selected`, glyph `background`→`foreground`) — ⬜
