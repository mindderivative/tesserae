...
| M41 — Widgets II: Composed Catalog and Overlays | `███░░░░░░░` 33% | 🚧 In progress — Phase 2 of 6 done (2026-09-25) |
...
**Just closed:** M41 Phase 2 (2026-09-25) — icon buttons, FABs, extended FABs, the split button (hover morph) and the button group (press morph and reflow), built from their fragments; 1198 → 1205. Before that, M41 Phase 1 (same day) — the `Widget` base and `button`.
...
**Status: 🚧 In progress — Phase 2 of 6 done (2026-09-25).** User: "Push it and go with your recommendations for M41" — Q1–Q4 as recommended (built from the fragments, MD3's behaviour, `tesserae.overlays`, the window-theme removal at M42's end). Rebuilds the composed MD3 factories and the overlays that `tre` M99 deletes, on 0.3.4's building blocks, M39's interaction and M40's controls. Scoped from `src/tesserae/widgets` (a scan of its `tre` calls), the fragments in `spec/components/`, `tre`'s `legacy-behavior.md` (0.3.5), and 0.3.4's `show_layer`/`hide_layer`.
...
**Status: ⬜ Proposed — approved, not started (2026-09-25).** The rest of the catalog.
