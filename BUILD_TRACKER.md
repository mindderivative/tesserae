...
**Just closed:** M27 — user-directed: "Start with M27 and then move onto M28." `tre`'s own side landed first, as its own M84 (`tre`'s `BUILD_TRACKER.md`) -- verified against this repo's own M26 scoping before writing anything, confirmed accurate, plus one real gap M26 missed (`Md3Baseline`'s fallback colors live in a crate `engine-spec` can't reach, resolved with a small local copy of just the 4 values needed). Once that landed, all 7 `*_Component.yaml` fragments were authored here (`RadioButton`/`Switch`/`CircularProgress`/`LinearProgress`/`LoadingIndicator`/`TimePickerDial`/`Link`), each compared directly against its own real imperative-catalog counterpart. One real bug caught here, not on the `tre` side: `Switch`'s first fragment draft named its toggle param `on` -- PyYAML's default loader treats a bare `on`/`off` as a YAML 1.1 boolean literal (confirmed directly), silently breaking `{{ on }}` substitution; renamed to `is_on`. A second real, small `tre`-side fix (`Node.get_text()` had no `Link` arm at all, found while writing this milestone's own `Link` test) shipped alongside M84 too.

**Previously:** M15-M26 — the macro-expansion engine, its wiring, 7 full/partial widget categories (59 fragments), the M25 re-scoping correction (60 fragments), and M26's own full technical scope for both real remaining fronts. See their own entries below.

**Up next:** M28 (macro-layer loop/repeat construct) is next, per the user's own explicit ordering. Real staging question already identified in M26: build it for `list_` alone first (clean, full-fidelity, reuses the already-shipped `ListItem` fragment) versus also reaching for `tabs`/`button_group`/`navigation_rail`/`navigation_drawer`/`menu` (needs a second, separate new capability -- conditional per-item styling -- for full fidelity). Not started yet.
