...
### Phase 1 — Decisions ⬜
- Step 1: the scrim fix. **Recommendation: `background: "#00000052"`** (black at 32%, `opacity` removed): MD3's scrim role is black in every generated scheme, so this matches `tre`'s own imperative scrim, with one simple edit per fragment. The cost is that a custom theme overriding `colors: {scrim: ...}` wouldn't reach these two fragments. Alternatives: restructure each fragment so the scrim is a sibling of the panel (keeps the theme role, but needs overlay positioning and changes the fragments' shape and ids), or ask `tre` for a role-with-alpha color syntax and wait — ⬜
- Step 2: the six labels. **Recommendation: `"#FFFFFF"`**, which keeps exactly what they look like today — ⬜
- Step 3: the local dev environment. **Recommendation: pin `.venv` to the `tre` v0.3.4 release**, as M32 did for v0.3.3 (the wheel is already built from the tag) — ⬜

### Phase 2 — Migrate ⬜
- Step 1: pin `.venv` to the v0.3.4 wheel; baseline 1 failed — ⬜
- Step 2: the two scrim fragments and six labels, per Phase 1's decisions; update each fragment's header comment and the side-sheet test to compare what now matches (scrim opacity 1.0 on both, the scrim's translucency in its color) — ⬜
- Step 3: tests that fail on the old YAML: no Text glyph color in Tesserae's own views, fragments or docs snippets has alpha below `FF`, and each overlay fragment's scrim and panel both have opacity 1.0 — ⬜
