...
| M32 — Migrate to `tre` 0.3.3 | `██████░░░░` 60% | 🚧 In progress — Phase 3 of 5 done (2026-09-25) |

**Just closed:** M32 Phase 3 (2026-09-25) — `tesserae.widgets` follows `tre` 0.3.3's names (`switch(selected=)`, `divider(orientation=)`, `link(content)`, `dialog(supporting_text)`, `toolbar(vibrant=)`; the `foreground=` → `color=` translations are gone), the tests follow, and the widget catalog lists the renames. On the pinned `tre` 0.3.3: **233 passed, 0 failed** (81 before M32), all 3 examples clean. Phase 2 (same day): `.venv` pinned to the v0.3.3 wheel, `tre`'s migration script run and reviewed (one bug in it caught). Before that, M30 (2026-09-25) — theme arguments on `App`: an app-wide theme on `App(...)` (`theme_seed`/`dark`/`default_theme`/`custom_theme`, plus `*_spec=` forms), a default stylesheet on `App(...)` that `App.load(..., stylesheet=)` overrides per screen, and `App.build_view()` for `register()`ed screens. Theme is app-wide because `tre` shares the first screen's theme with the window. Theme-file hot reload was split out into M31. 221 → 233 tests, verified against `tre` v0.3.2 built from source (this repo's `.venv` currently loads `tre`'s unreleased 0.3.3).
...
**Up next:** M32 Phase 4 — move CI's `tre` pin to `v0.3.3` and push (Phases 2–3 are held locally until then, since CI still tests v0.3.2), then confirm `tre` M91's fix reaches Tesserae's hot reload; waiting on the user's go-ahead. Then Phase 5 (closing the known gaps 0.3.3 fixes), then M31.
...
**Status: 🚧 In progress — Phase 3 of 5 done (2026-09-25).** User-directed: "scope the 0.3.3 migration as M32"; decisions: "go with your recommendations for M32". `tre` v0.3.3 is released (tag on `bc5e9b6` on `tre`'s `main`, 24 release assets; verified), carrying M90's schema and API renames (full table in `tre`'s `docs/migrating-0.3.3.md`), M91's fix for `tre` issue #8 plus `View.set_stylesheet`, and M92's animatable icon color. Unblocks M31.
...
### Phase 4 — Verify ⬜
- Step 1: full suite against `tre` v0.3.3 (81 failing today → 0) and all 3 examples (all failing today); mutation-check anything new — ⬜
- Step 2: move CI's `tre` pin from `v0.3.2` to `v0.3.3`; CI green — ⬜
- Step 3: confirm M91's fix reaches Tesserae: a hot-reload edit to a bound node keeps its bound value (M29's known gap), and `View.set_stylesheet` is available for M31 — ⬜
