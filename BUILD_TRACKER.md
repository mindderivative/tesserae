...
- **`tre` 0.3.4 (released 2026-09-25, tag on `29800f3`; its M95) will break two things in Tesserae when it moves there — scoped as M33, which confirmed both against a real 0.3.4 build and found nothing else.** (1) A color's alpha now *multiplies* with the node's opacity instead of being replaced by it, and six places color a Text's glyph `"#FFFFFF00"` (white, alpha 0): `examples/counter/Counter_View.yaml`, `examples/multi_screen/Home_View.yaml` and `Settings_View.yaml`, `examples/todo_list/TodoItem_View.yaml`, `docs/getting-started.md`, `docs/guide/components.md`. Those labels paint solid white today and become **invisible** on 0.3.4. They need a real opaque glyph color (still fine through 0.3.3's `foreground` rename). (2) `opacity` becomes *group* opacity, and `Dialog_Component.yaml`/`SideSheetModal_Component.yaml` put `opacity: 0.32` on a scrim whose child is the panel, so the panel would fade to 32% too. Restructure with the scrim as a sibling, or use a translucent scrim color. `animate()`'s new argument order doesn't affect Tesserae (no positional `on_complete`). For the migration after M32.
- **A background thread can free a main-thread `tre` object and trigger a panic.** Found in CI run 36098471205 (`8de5e8c`, 2026-09-25) as 2 `PytestUnraisableExceptionWarning`s in `tests/test_watch_thread.py`: while the `ViewWatcher` thread was parsing YAML, the allocation set off Python's cyclic garbage collector *on that thread*, which freed a `View` whose last reference sat in a reference cycle (a re-raised exception's traceback keeping the previous test's frame, and its `view`, alive). `tre`'s `View` is `unsendable`, so pyo3 raised `RuntimeError: ... View is unsendable, but is being dropped on another thread` and a `PanicException` in `tp_clear`; Python reports both as ignored exceptions and the `View` is leaked or half-cleared. Reproduced deterministically on `tre` v0.3.2 and 0.3.3 with a plain Python cycle holding a `View` plus `gc.collect()` on a background thread. (Correction: a View/ViewModel pair on its own is *not* such a cycle — those links live in Rust, where the collector can't see them.) The watcher never touches a view from its thread, but that can't prevent this; any app using a background thread, including `tre`'s own `thread_handle` pattern, can hit it whenever a `View` becomes garbage through a cycle. No Tesserae-side workaround found. Filed as [`tre` issue #10](https://github.com/mindderivative/tre/issues/10) (user-directed), suggesting that `unsendable` objects defer their drop to their own thread, or at least don't panic in `tp_clear`. **Fixed in `tre` 0.3.4** (M96 Phase 1; released 2026-09-25, issue #10 closed): every `tre` object can be dropped on any thread, with the free finished on its owning thread at the next frame or `tre` call. Using one from another thread still raises. Closes when Tesserae moves to 0.3.4. Verified on a 0.3.4 build (M33 scoping): the deterministic repro that raises `PanicException` + `RuntimeError` on 0.3.3 raises nothing on 0.3.4. Closes with M33.
...
- Not published to PyPI. `tre` v0.3.3 is a published GitHub Release with 24 attached assets (`github.com/mindderivative/tre/releases/tag/v0.3.3`, 2026-09-25), and both Tesserae's CI and its local `.venv` pin it, but neither `tre` nor Tesserae is on PyPI yet.
...
- Step 1: full suite and all 3 examples on the pinned 0.3.4; a regression test that a background-thread `gc.collect()` freeing a `View` raises nothing (`tre` issue #10, which failed on 0.3.3); CI's `tre` checkout moved to `v0.3.4` and green — ⬜
- Step 2: docs (`installation.md` → v0.3.4; `guide/themes-and-fonts.md` or the widget catalog on color alpha and group opacity; the two snippets); known gaps: the 0.3.4 gap and the GC gap move to "Fixed gaps", the PyPI gap updated; tracker and artifact; `PLAN.md`/`LOG.md` — ⬜
