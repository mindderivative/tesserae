...
- **A background thread can free a main-thread `tre` object and trigger a panic.** Found in CI run 36098471205 (`8de5e8c`, 2026-09-25) as 2 `PytestUnraisableExceptionWarning`s in `tests/test_watch_thread.py`: while the `ViewWatcher` thread was parsing YAML, the allocation set off Python's cyclic garbage collector *on that thread*, which freed an unrelated `View` created on the main thread (a `View` and its `ViewModel` reference each other, so a discarded pair waits for the cycle collector). `tre`'s `View` is `unsendable`, so pyo3 raised `RuntimeError: ... View is unsendable, but is being dropped on another thread` and a `PanicException` in `tp_clear`. Python reports both as ignored exceptions and the `View` is leaked or half-cleared. The watcher never touches a view from its thread, but that doesn't prevent this: *any* Python thread that allocates can end up collecting another thread's garbage, including in `tre`'s own recommended `thread_handle` pattern. Rare in a real app (views are long-lived; `component.remove()`'d components are the likely case), timing-dependent in tests (not reproduced locally). Likely needs a `tre`-side fix (unsendable objects that tolerate cross-thread cyclic GC, e.g. deferring the drop to their own thread); not yet filed as a `tre` issue.
