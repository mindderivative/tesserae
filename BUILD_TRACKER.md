...
**Up next:** Nothing currently scoped. Coming, in order: (1) **`tre` v0.3.3 is released** (tag on `bc5e9b6` on `tre`'s `main`, GitHub release published 2026-09-25 with 24 assets; verified), so a Tesserae **0.3.3 migration** milestone can now be scoped (see its known gap; `tre` ships `docs/migrating-0.3.3.md` and `tools/migrate_views_0_3_3.py`), including moving CI's `tre` pin from `v0.3.2` to `v0.3.3` — the user's call. Then **M31** (hot reload for theme and stylesheet files, unblocked by `tre` M91's fix for issue #8). (2) **`tre`'s building-block program (M93–M103), approved by the user in the `tre` session 2026-09-25:** `tre` drops its declarative layer, reactivity, MD3 components and theming, and Tesserae takes them over, built on `tre`'s imperative primitives. The additive building blocks ship as `tre` 0.3.4 with the old API still present; removals and renames ship as 0.3.5, and nothing is removed until Tesserae confirms it no longer uses it. Tesserae's needs (a keyed reconciler with reorder, root swap, batch updates, the `{{ }}` evaluator, resize/theme-switch events, text measurement) were sent for `tre`'s M93 spec, which comes here for review before anything is built. A Tesserae-side program for this is not yet scoped. Other named, un-scoped candidates: hot reload for `App.register()`ed screens, and conditional per-item styling for the 5 Rust-internal-state-dependent-coloring widgets.
