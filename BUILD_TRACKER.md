...
| M37 — Declarative Engine on `tre` Primitives | `██░░░░░░░░` 17% | 🚧 In progress — Phase 1 of 6 done (2026-09-25) |
...
**Status: 🚧 In progress — Phase 1 of 6 done (2026-09-25).** User: "push it and scope M37", then "push it and go with your recommendations for M37" — Q1–Q4 as recommended: tokens first (this milestone's Phase 1), the nine MD3 kinds through the legacy factories until M40, `tesserae.View` with today's surface plus `root`, and a tree differ for parity. Replaces `View`, `reconcile`, `instantiate`/`Component`, the style cascade, `tre`'s binding wiring (moved here from M36) and `Window.from_view`/`show_view`. The largest milestone in the program.
