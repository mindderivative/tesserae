...
| M29 — Tesserae Owns All File Handling; `tre` Gets Specs + Bytes Only | `████████░░` 80% | 🚧 In progress — Phases 1–4 of 5 done (2026-09-24) |

**Just closed: M27 and M28**, both in one real pass — user-directed: "Start with M27 and then move onto M28."

**M27** — `tre`'s own side landed first, as its own M84 (`tre`'s `BUILD_TRACKER.md`) -- verified against this repo's own M26 scoping before writing anything, confirmed accurate, plus one real gap M26 missed (`Md3Baseline`'s fallback colors live in a crate `engine-spec` can't reach, resolved with a small local copy of just the 4 values needed). Once that landed, all 7 `*_Component.yaml` fragments were authored here (`RadioButton`/`Switch`/`CircularProgress`/`LinearProgress`/`LoadingIndicator`/`TimePickerDial`/`Link`), each compared directly against its own real imperative-catalog counterpart. One real bug caught here, not on the `tre` side: `Switch`'s first fragment draft named its toggle param `on` -- PyYAML's default loader treats a bare `on`/`off` as a YAML 1.1 boolean literal (confirmed directly), silently breaking `{{ on }}` substitution; renamed to `is_on`. A second real, small `tre`-side fix (`Node.get_text()` had no `Link` arm at all, found while writing this milestone's own `Link` test) shipped alongside M84 too.
...
**Up next:** M29 — Tesserae takes over all file handling (reading, parsing, decoding, watching) and hands `tre` only specs and bytes, per a user-directed handoff from the `tre` session (2026-09-24). Scoped below. **Phases 1–3 done** (user go-aheads 2026-09-24: "pin CI to 0.3.2 and start Phase 1", "push it and use Pillow for Phase 2", "push it and use a poll loop for Phase 3", "push it and start Phase 4"). Tesserae now gives `tre` no file path anywhere, and reads theme, stylesheet and font files itself. Only Phase 5 (a final docs pass) remains. Separately, the 5 Rust-internal-state-dependent-coloring widgets (`tabs`/`button_group`/`navigation_rail`/`navigation_drawer`/`menu`) remain a real, named, un-scoped future candidate for whenever conditional per-item styling gets designed -- not started.
...
**Known gaps:**
- Hot reload (`tesserae.spec.ViewWatcher`, M29 Phase 3) can't run inside `App.run()`: `tre`'s `View` is single-threaded and its event loop has no periodic Python hook, so there's nowhere to call `poll()` from. Works in an app-controlled loop today; needs a `tre`-side periodic event-loop callback — requested as [`tre` issue #6](https://github.com/mindderivative/tre/issues/6) (2026-09-24, user-directed). **Unblocked 2026-09-25:** `tre` M87 (`origin/0.3.2` tip `0066203`, issue #6 closed) adds `App.thread_handle() -> LoopHandle` with a thread-safe `call_soon(fn)` that wakes even an idle loop and runs `fn` on the event-loop thread — a callback queue rather than the issue's proposed `set_interval` (the user's choice in the `tre` session). Verified here: the API exists in this repo's `.venv`, and a non-callable raises `TypeError`. Not used by Tesserae yet; CI's `tre` pin moved from `d6c30ef` to `0066203` (relayed from the `tre` session as user-directed there; verified `0066203` is `d6c30ef` plus only M87's three commits, and pushed). The `tre` session relays that the user wants Tesserae's watcher driven by file-change events (e.g. `watchfiles`) — a change to this session's own user-chosen poll loop and a new dependency, so waiting on the user's confirmation here before building it.
...
- `App`/`App.load()` has no theme-related API at all (`theme_seed`/`custom_theme`/`dark`/`stylesheet`) — a real, pre-existing gap, newly relevant now that `component:` fragments can reference MD3 color roles/shape tokens through `App.load()`. `load_view` itself takes them all since M29 Phase 4, reading theme/stylesheet files in Tesserae; a themed screen can use `load_view(...)` + `App.register(...)` meanwhile.
- Hot reload (`ViewWatcher`) doesn't watch theme or stylesheet files (M29 Phase 4); a theme edit needs an explicit `set_theme` call with a freshly loaded theme.
...
- Not published to PyPI. `tre` v0.3.1 is now a real, tagged GitHub Release with cross-platform wheels attached (`github.com/mindderivative/tre/releases/tag/v0.3.1`), but neither `tre` nor Tesserae is on PyPI itself yet -- both still depend on a local editable checkout for actual development.
