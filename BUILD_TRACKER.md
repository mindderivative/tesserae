...
**Up next:** **M31 Phase 2** (stylesheet files: watch the app's default stylesheet and each screen's own, re-apply with `View.set_stylesheet`, and settle the two M91 consequences — `on_change` firing once per live update, and embedded components not re-themed), then Phase 3 (live test, final docs). After that, **`tre`'s building-block program**: `tre` 0.3.4 is released (2026-09-25, tag on `29800f3`: the additive building blocks, M93–M96, with the old API still present), and its M97 gate waits on Tesserae taking over `tre`'s declarative layer, reactivity, MD3 components and theming — a large Tesserae-side program, not yet scoped (known gaps list what 0.3.4 will break). Other named, un-scoped candidates: hot reload for `App.register()`ed screens, and conditional per-item styling for the 5 Rust-internal-state-dependent-coloring widgets.
...
- Hot reload doesn't watch stylesheet files yet (M29 Phase 4); after editing one, call `view.set_stylesheet(stylesheet_spec=load_stylesheet(...))`. Theme files are watched since M31 Phase 1. Stylesheets are **M31 Phase 2**, using `tre` 0.3.3's `View.set_stylesheet` (M91).
...
- ~~Hot reload didn't watch theme files.~~ **Fixed (M31 Phase 1).** `run(hot_reload=True)` watches them and re-themes every screen and the window.
