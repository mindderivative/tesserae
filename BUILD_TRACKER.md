...
**Just closed:** M26 — full technical scope for both real fronts M25 identified, as two separate future milestones (neither built yet). Front A (`tre`-side, tentatively M27): sized all 7 primitives' real constructors against `WidgetSpec`, and found something M25's own summary missed — 5 of the 7 (`radio_button`/`switch`/`circular_progress`/`linear_progress`/`time_picker_dial`) hardcode `PaintProperties.background` to `TRANSPARENT` and carry their entire real visual in internal per-kind tint fields (`unselected_tint`, `track_on_tint`, `indicator_tint`, etc.) that `Checkbox`/`Slider`'s own existing declarative precedent never had to theme-resolve, since neither has one. Confirmed a small reusable role-resolve-with-fallback helper (built on the already-existing `resolve_color`/`ColorScheme::role`) closes this cleanly — bounded, no new subsystem. Also found `loading_indicator` needs an extra required step (`paint.shape` seeded from `state.shapes[0]`) to avoid a real, confirmed first-tick flash bug, and `time_picker_dial` is the one primitive needing genuinely new `WidgetSpec` fields (`hour: u8`/`minute: u8` — not the `mode` field guessed in M25, corrected after reading the real constructor). Front B (`tre`-side loop construct, tentatively M28): found the 6 dynamic-list widgets aren't uniform — `add_list` takes pre-built `Node`s (pure layout composition, the loop construct's cleanest fit, repeating the already-shipped `ListItem` fragment); `add_tabs`/`add_button_group`/`add_navigation_rail`/`add_navigation_drawer`/`add_menu` take raw `Vec<String>` and construct real state-dependent active/inactive coloring internally in Rust, which a static `{{ }}`-substitution template can't branch on — so a loop construct alone gives full-fidelity coverage for `list_` but only partial/simplified coverage for the other 5 without a second, separate new capability (conditional per-item styling). Checked pyCopper (this whole macro layer's own stated precedent) directly — it has no repeat/loop construct at all, confirming this is genuinely new design territory, not something to port. Neither front built — scope only, per the user's own instruction.

**Previously:** M15-M25 — the macro-expansion engine, its wiring, 7 full/partial widget categories (59 fragments), and the M25 re-scoping correction (60 fragments). See their own entries below.

**Up next:** Both fronts are now fully scoped (see Milestone 26 below for the full technical design of each) but neither is built. M27 (`tre`-side primitives) is the lower-risk, more bounded of the two — same additive-only shape as the already-proven M74 `Icon` precedent, no architecture changes. M28 (macro-layer loop construct) is bigger and has a real staging question baked in: build it for `list_` alone first (clean, full-fidelity, reuses the already-shipped `ListItem` fragment) versus also reaching for `tabs`/`button_group`/`navigation_rail`/`navigation_drawer`/`menu` (needs a second, separate new capability — conditional per-item styling — for full fidelity). Neither started yet.
...
**Known gaps:**
- 60 of ~68 real MD3 widgets now have a declarative `*_Component.yaml` fragment; the remaining 8 are either dynamic/runtime-constructed or block on a `tre`-side primitive that doesn't exist yet (see below) — not mechanical follow-up, a real structural boundary.
