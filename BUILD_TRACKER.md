...
- Hot reload (`ViewWatcher`) doesn't watch theme or stylesheet files (M29 Phase 4); a theme edit needs an explicit `set_theme` call with a freshly loaded theme. Theme files: **scoped as M30 Phase 3**. Stylesheet files: blocked — `tre` has no way to change a `View`'s stylesheet after construction (no `set_stylesheet`).
...
- Step 3: whether Phase 3 (hot reload of theme files) is in scope for this milestone, or a follow-up — ⬜
...
- Step 3: stylesheet files stay unwatched — record the missing `tre`-side `set_stylesheet` as a known gap (and a candidate `tre` issue, if the user wants one filed) — ⬜
