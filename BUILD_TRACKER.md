...
**Known gaps:**
- Hot reload (`tesserae.spec.ViewWatcher`, M29 Phase 3) can't run inside `App.run()`: `tre`'s `View` is single-threaded and its event loop has no periodic Python hook, so there's nowhere to call `poll()` from. Works in an app-controlled loop today; needs a `tre`-side periodic event-loop callback — requested as [`tre` issue #6](https://github.com/mindderivative/tre/issues/6) (2026-09-24, user-directed).
