...
- `App.load()`/`tesserae.instantiate()` don't use `load_view`/macro-expansion yet — a real app must call `tesserae.spec.load_view(...)` directly today.
- `tesserae.instantiate()` (embedded components) can't use `component:` at all — blocked on a `tre`-side `Component.instantiate(source=...)` addition.
