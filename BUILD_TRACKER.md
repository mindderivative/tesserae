...
**Up next:** Nothing currently scoped. M31 (hot reload for theme and stylesheet files) is written up but waits on Tesserae moving to `tre` 0.3.3, where [`tre` issue #8](https://github.com/mindderivative/tre/issues/8) is fixed (M91); that 0.3.3 migration is the natural next milestone to scope once 0.3.3 is released (see its known gap). Other named, un-scoped candidates: hot reload for `App.register()`ed screens, and conditional per-item styling for the 5 Rust-internal-state-dependent-coloring widgets (`tabs`/`button_group`/`navigation_rail`/`navigation_drawer`/`menu`).
