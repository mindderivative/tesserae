...
**Just closed: M29** — Tesserae owns all file handling; `tre` gets specs and bytes only. User direction, relayed from the `tre` session: "Tesserae should not be pushing files directly to tre. It should be pushing spec information and handling the files itself." Built in six user-approved steps (Phases 1, 2, 3, 3b, 4, 5), each verified against `tre`'s real source before writing code, and each mutation-checked:
...
**Up next:** Nothing currently scoped. Named, un-scoped candidates: theme arguments on `App.load()` and hot reload for theme/stylesheet files (both recorded in M29's own limits); `App.register()` screens can't hot-reload since Tesserae doesn't know their file; and the 5 Rust-internal-state-dependent-coloring widgets (`tabs`/`button_group`/`navigation_rail`/`navigation_drawer`/`menu`), for whenever conditional per-item styling gets designed.
