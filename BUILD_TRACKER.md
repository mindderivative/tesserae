...
- Step 2: CI's `tre` checkout moved from `v0.3.3` to `v0.3.4` (`ci.yml`, comment updated). Green run not yet seen: it needs a push, and this phase's commits aren't pushed. CI on 0.3.3 would fail the updated overlay tests, so push M33 together — 🚧
