...
| M36 — Bindings and Handlers in Tesserae | `█████░░░░░` 50% | 🚧 In progress — Phase 1 of 2 done (2026-09-25) |
...
**Up next:** **M36** (bindings and handlers in Tesserae: port `binding.rs`'s grammar, wire bindings, `two_way:` and handlers, and drop M35's bridge to `tre`'s recording stack) — approved, waiting on the user's go-ahead. `tre` is building the migration guide (including an audit of `binding.rs` against its doc comments, useful for M36) and the `TRE_FORBID_REMOVED` gate shim as its M97 Phase 2. Other named, un-scoped candidates: hot reload for `App.register()`ed screens, and conditional per-item styling for the 5 Rust-internal-state-dependent-coloring widgets (likely absorbed by M40, since Tesserae will own those widgets).
...
**Status: 🚧 In progress — Phase 1 of 2 done (2026-09-25).** User: "push it and start M36". `binding.rs`'s evaluator moves to Tesserae (`tre` M98).
