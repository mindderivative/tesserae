...
**Status: ⬜ Not started — scoped (2026-09-25), awaiting user decisions.** User-directed: "scope the 0.3.3 migration as M32". `tre` v0.3.3 is released (tag on `bc5e9b6` on `tre`'s `main`, 24 release assets; verified), carrying M90's schema and API renames (full table in `tre`'s `docs/migrating-0.3.3.md`), M91's fix for `tre` issue #8 plus `View.set_stylesheet`, and M92's animatable icon color. Unblocks M31.
...
### Phase 1 — Decisions (user) ⬜
- Step 1: Tesserae's public widget names. **Recommended:** follow `tre` 0.3.3 — `tesserae.widgets` has always mirrored `tre`'s `add_*` parameters, and several of Tesserae's own translations become unnecessary (`icon`/`loading_indicator` already say `foreground=`). Concretely: `switch(selected=)`, `divider(orientation=)`, `link(content=)`, `dialog(supporting_text=)`, `toolbar(vibrant=bool)` in place of `tone=`, and the `"value"` property for sliders. It's a breaking change for Tesserae's own callers, acceptable while pre-alpha. Alternative: keep Tesserae's current names and translate them to `tre`'s new ones — ⬜
- Step 2: `Switch_Component.yaml`'s `is_on` parameter. **Recommended:** rename to `selected`, matching `RadioButton` and `tre`'s new field — ⬜
- Step 3: the local dev environment. Today `.venv` loads `tre` from `tre`'s source tree, so every `tre` rebuild (now on the 0.3.4 branch) changes what Tesserae's local tests see. **Recommended:** pin `.venv` to the `tre` v0.3.3 release (the same one CI pins), and test against `tre`'s source tree only deliberately. Alternative: keep tracking the source tree and keep verifying in a scratch venv, as M30 did — ⬜

### Phase 2 — YAML Migration ⬜
- Step 1: run `tools/migrate_views_0_3_3.py` over `src/` and `examples/`, and over the 8 `.py` files with embedded YAML; review every diff (comments preserved) and commit it on its own so the mechanical change is reviewable apart from the hand edits — ⬜
- Step 2: hand-migrate what the script can't reach: Python dict specs in the tests, and the fragments' `params:`/comments that name old fields (e.g. `Switch_Component.yaml`'s header, per Phase 1 Step 2) — ⬜
