...
- **Upcoming `tre` 0.3.3 breaks Tesserae's YAML.** `tre`'s M90 (commit `a5bed0a` on its unreleased `0.3.3` branch, seen 2026-09-25) renames the declarative schema: enum values become lowercase snake_case (`flex_direction`, `fit`, `align_items`, `justify_content`), a new `style.foreground` sets Text/Link/Icon/LoadingIndicator glyph color (inline `style.background` on those becomes an error), `Switch`/`RadioButton` take `selected:` instead of `checked:`, and `text.role` becomes `text.typography_role`. 37 of the 67 fragments use `role:` alone; the other renames reach most fragments, examples and test views. Nothing breaks while CI pins `v0.3.2`; moving to 0.3.3 needs a migration milestone. **For that milestone** (per the `tre` session, 2026-09-25): `tre` ships `tools/migrate_views_0_3_3.py` (same M90 commit, not yet pushed), which rewrites view and stylesheet YAML in place, keeping comments — including YAML inside `.py` strings — and supports `--check`. It covers the enum, `typography_role`, `foreground` (only on Text/Link/Icon/LoadingIndicator), `selected` (only on Switch/RadioButton, including binding keys and `two_way`) and Slider `thumb_position` → `value` renames. It does **not** touch Python calls. That matters here, because `tesserae.widgets` mirrors `tre`'s `add_*` parameters and several of those change too: `foreground=` on `add_text`/`add_icon`/`add_loading_indicator` (Tesserae's `icon`/`loading_indicator` currently translate `foreground=` to `color=`), `add_switch(selected=)` with `Node.set_selected`/`get_selected` (`set_on`/`get_on` removed), `add_toolbar(vibrant=bool)` (Tesserae's `toolbar(tone=)` translates to `color=` today), `orientation=` on `add_divider`/`add_scroll_view`, `add_link(content=)`, `supporting_text=` on `add_dialog`/`add_popover`, and the `thumb_position` → `value` and text-widget `background` → `foreground` property names used by tests. `tre` will publish a full migration table in its docs (M90 Phase 3). (Seen early because this repo's `.venv` imports `tre` from its source tree: the `tre` session's rebuild at 21:27 made 81 local tests fail against the unfinished M90 — not a Tesserae change.)
