...
| M39 — Interaction: State Layer, Ripple, Focus, Accessibility | `██░░░░░░░░` 25% | 🚧 In progress — Phase 1 of 4 done (2026-09-25) |
...
**Just closed:** M39 Phase 1 (2026-09-25) — clickable YAML nodes are keyboard-reachable buttons again (an M37 regression, fixed); `tre`'s M97 Phase 2 Step 6 dry run reported to `tre`: `src/tesserae` uses nothing M98 removes; 1042 → 1048. Before that, M38 (same day) — the MD3 theme is Tesserae's: `tesserae.Theme` (matching `tre`'s `Window.theme` on 109 lookups, plus motion tokens), theme typography for display text in views, and light and dark with `App(dark="system")` by default; 909 → 1042 over the milestone. Phase 3 (same day) — light and dark: `App(dark="system")` by default, starting dark and following the OS through `color_scheme`, fixed choices re-asserted, `App.set_dark`; 1029 → 1042. Before that, M38 Phase 2 (same day) — a theme's `typography:` now styles display text in views (Text and Link), not text inputs, per Q1; 1019 → 1029. Before that, M38 Phase 1 (same day) — `tesserae.Theme`: roles, `components:` shape and elevation, `typography:` overrides and MD3's motion tokens, matching `tre`'s `Window.theme` on 109 lookups; `app.theme`/`view.theme`; 909 → 1019. Before that, M37 (same day) — Tesserae builds, reconciles and wires every view itself on `tre` 0.3.4's building blocks: tokens, compiler and cascade (127 views identical to `tre`'s), `tesserae.View` with the reconciler and Tesserae's own bindings, components with their host's theme, `App` screens in its own window, `load_view` and the whole suite off `tre`'s `View`, and M35's bridge removed; 741 → 909 tests over the milestone. Phase 5 (same day) — components and screens on Tesserae's builder: components get and follow their host's theme and stylesheet (M31's gap closed), and `App` builds screens in its own window and switches them by attaching and detaching; `tesserae.View`/`Component` are Tesserae's; 900 → 913, all examples on Tesserae-built views. Before that, M37 Phase 4 (same day) — `tesserae.View`: Tesserae builds, reconciles (keeping identity and focus, ordering children, unlike `tre`) and wires bindings, handlers and `two_way:` itself, with `on_change` only for user edits; error messages equal `tre`'s; 870 → 900. Before that, M37 Phase 3 (same day) — the tree differ: 127 views (all 67 fragments, the suite's inline views, the examples, the docs) built by `tre` and by Tesserae are identical node by node; it found and fixed the TextField text colour, the MD3 kinds' paint, and root sizing; 741 → 870. Before that, M37 Phase 2 (same day) — Tesserae's spec compiler and cascade: every kind onto 0.3.4's primitives (the eight MD3 kinds through legacy factories until M40), `tre`'s cascade with indexed rules, and `tre`'s exact error messages, checked against `tre`'s builder (42 tests); 698 → 741. Before that, M37 Phase 1 (same day) — `tesserae.tokens`: MD3 colour roles, shape, elevation-as-shadows, type scale and colour parsing, each checked against `tre` 0.3.4 (74 tests); 623 → 698. Before that, M36 (same day) — the evaluator, and a decision: `tre`'s new API can't drive the legacy Checkbox/Switch state, so wiring moves to M37 (bindings on `checked`/`selected`/`value` to M40). Phase 1 (same day) — `tesserae.binding`, a port of `tre`'s `{{ }}` evaluator that gives `tre`'s exact result or error for all 166 expressions tested (every rule, plus every binding in the repo); new Binding Expressions guide; 455 → 623 tests. Before that, M35 (same day) — Tesserae owns its reactivity: `tesserae.reactive` (taken over from `tre`, same behaviour, proved by one suite run against both), with a bridge feeding `tre`'s binding tracking until M36; 426 → 455 tests. Before that, M34 (same day) — the building-block program's design gate: design page, decisions P1–P8, `tre` agreed the gate check and migration guide, and three spikes (colour parity 784/784; Python cascade and bindings build 4,000 nodes in 20.7 ms against `tre`'s 11.0 ms, with live updates on par; a full MD3 checkbox on primitives, 12/12 checks), kept in `tools/spikes/`. Before that, M44 (same day) — Tesserae logs through loguru: `tesserae.configure_logging()` for the console, INFO for hot reloads, ERROR (traceback at DEBUG) for a failed background reload instead of raising through `tre`, Python warnings shown through loguru, the examples on `logger`; 419 → 426 tests. Before that, M34's color spike (same day) — `materialyoucolor` reproduces `tre`'s MD3 schemes exactly (784/784 roles, 8 seeds, light and dark) with `spec_version="2021"` and tone 10 for the four light `on_*_container` roles, so P2 is accepted; the migration-guide and gate-check message went to `tre`. Before that, M34 Phase 1 Steps 1–2 (same day) — the building-block program is approved with all eight recommendations; `docs/design/building-blocks.md` written; `tre`'s reactivity found to be pure Python already, so M35 shrinks. Before that, M33 (2026-09-25) — Tesserae migrated to `tre` 0.3.4: `.venv` and CI both pin the v0.3.4 release; the scrim fragments and six labels fixed for its paint model (color alpha renders, `opacity` is group opacity), guarded by a scan of every view, fragment and docs snippet; `tre` issue #10's fix lands, with a regression test. 419 passed locally; CI green on v0.3.4 (417 passed, 2 skipped for no display). Phase 3 (same day): the tre#10 test, CI pin, install docs, a transparency section. Phase 2 (same day) — Tesserae runs on `tre` 0.3.4: `.venv` pinned to the release; the two scrim fragments take their 32% from their color (`"#00000052"`, matching `tre`'s own scrim exactly) so group opacity doesn't fade their panels; the six invisible `"#FFFFFF00"` labels are `"#FFFFFF"`; a scan of every view, fragment and docs snippet guards both. 418 passed. Before that, M31 (2026-09-25) — hot reload for theme and stylesheet files, complete. `run(hot_reload=True)` now watches the app's theme files, its default stylesheet and each screen's own stylesheet, and re-styles the running app in place with bound values kept (`App.set_theme_specs()` and `App.set_stylesheet_spec()` are public). It also fixed an M30 bug (the window was never themed) and filed [`tre` issue #12](https://github.com/mindderivative/tre/issues/12) (`on_change` firing on every reload). Phase 3 added a live `App.run()` test and the final docs. 234 → 255 tests. Phase 2 (same day) — stylesheet files hot-reload: the default from `App(stylesheet=)` through the new `App.set_stylesheet_spec()`, and each screen's own file for the screens built with it; a failed re-style puts back the screens already changed. The two M91 consequences were probed and documented: `on_change` fires once per reload on bound nodes, and embedded components never get the host's theme or stylesheet (a `tre` limitation). 245 → 254 tests. Before that, M31 Phase 1 (same day) — theme files hot-reload: `run(hot_reload=True)` watches `App(default_theme=, custom_theme=)` and re-themes every screen and the window through the new `App.set_theme_specs()`. Along the way it found an M30 bug: the window was never themed, because building a `View` doesn't set the window's theme. Imperative widgets and interaction tints ran unthemed until now; `App.show()` now themes the window, matching the view's seed and colors. 234 → 245 tests. Before that, M32 (2026-09-25) — Tesserae migrated to `tre` 0.3.3: `.venv` and CI both pinned to the v0.3.3 release; `tre`'s migration script run and reviewed (a bug in it caught — Rect `background`s rewritten inside `.py` strings); dict specs, docs and `Switch`'s `selected` done by hand; `tesserae.widgets` follows `tre` 0.3.3's names; a regression test proves bound values now survive hot reload (`tre` M91). 81 failing → 0; CI green (233 passed, 1 skipped for no display). Unblocks M31. Phase 3 (same day): `tesserae.widgets` follows `tre` 0.3.3's names; 233 passed, 0 failed on 0.3.3. Phase 2 (same day): `.venv` pinned to the v0.3.3 wheel, `tre`'s migration script run and reviewed (one bug in it caught). Before that, M30 (2026-09-25) — theme arguments on `App`: an app-wide theme on `App(...)` (`theme_seed`/`dark`/`default_theme`/`custom_theme`, plus `*_spec=` forms), a default stylesheet on `App(...)` that `App.load(..., stylesheet=)` overrides per screen, and `App.build_view()` for `register()`ed screens. Theme is app-wide because in `tre` a theme belongs to the window (M30 believed the window took the first screen's theme; M31 Phase 1 found it doesn't, and fixed it). Theme-file hot reload was split out into M31. 221 → 233 tests, verified against `tre` v0.3.2 built from source (this repo's `.venv` currently loads `tre`'s unreleased 0.3.3).
...
**Status: 🚧 In progress — Phase 1 of 4 done (2026-09-25).** User: "Yes" (scope M39), then "Yes do as recommended" — Q1–Q3 as recommended. `tre` D8 and R12: `tre` draws no ripple, state layer, scrim or focus ring. Scoped from `tre`'s new `docs/design/legacy-behavior.md` (its M97 Phase 2 Step 2, on its local `0.3.5` branch, `9f926d6`) and probes on 0.3.4.
