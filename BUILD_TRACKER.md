...
**Fixed gaps:**
- ~~Hot reload (`ViewWatcher`, M29 Phase 3) can't run inside `App.run()` — `tre`'s `View` is single-threaded and its loop had no way to run Python from another thread.~~ **Fixed (M29 Phase 3b).** `tre` M87 added `App.thread_handle()`/`LoopHandle.call_soon` ([`tre` issue #6](https://github.com/mindderivative/tre/issues/6)); `App.run(hot_reload=True)` now watches every `load()`ed screen with a `watchfiles`-driven background `ViewWatcher`.
...
### Phase 3b — Hot Reload Inside `App.run()` (`watchfiles` + `tre` M87) ✅
