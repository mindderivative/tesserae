...
**Up next:** Nothing currently started. **M31** (hot reload for theme and stylesheet files) is written up and now unblocked — waiting on the user's go-ahead. After that, **`tre`'s building-block program**: `tre` 0.3.4 is being released (its additive building blocks, M93–M96, all done), and its M97 gate waits on Tesserae taking over `tre`'s declarative layer, reactivity, MD3 components and theming — a large Tesserae-side program, not yet scoped (known gaps list what 0.3.4 will break). Other named, un-scoped candidates: hot reload for `App.register()`ed screens, and conditional per-item styling for the 5 Rust-internal-state-dependent-coloring widgets.
...
- Hot reload (`ViewWatcher`) doesn't watch theme or stylesheet files (M29 Phase 4); a theme edit needs an explicit `set_theme` call with a freshly loaded theme. **No longer blocked:** `tre` 0.3.3 keeps bound values through `set_theme` and adds `View.set_stylesheet` ([`tre` issue #8](https://github.com/mindderivative/tre/issues/8), M91), and Tesserae is on 0.3.3 since M32 — scoped as **M31**, not started.
