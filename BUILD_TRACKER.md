...
**Up next:** M32 Phase 2 — pin `.venv` to the `tre` v0.3.3 release, then the YAML migration; Phase 1's decisions are made (the user took all three recommendations, 2026-09-25). Waiting on the user's go-ahead to start Phase 2. After M32: **M31** (hot reload for theme and stylesheet files, unblocked by 0.3.3). Later: **`tre`'s approved building-block program (M93–M103)**, under which Tesserae takes over `tre`'s declarative layer, reactivity, MD3 components and theming (0.3.4 adds building blocks with the old API still present; 0.3.5 removes; nothing removed until Tesserae confirms) — a Tesserae-side program for it is not yet scoped. Other named, un-scoped candidates: hot reload for `App.register()`ed screens, and conditional per-item styling for the 5 Rust-internal-state-dependent-coloring widgets.
...
**Status: 🚧 In progress — Phase 1 of 5 done (2026-09-25).** User-directed: "scope the 0.3.3 migration as M32"; decisions: "go with your recommendations for M32". `tre` v0.3.3 is released (tag on `bc5e9b6` on `tre`'s `main`, 24 release assets; verified), carrying M90's schema and API renames (full table in `tre`'s `docs/migrating-0.3.3.md`), M91's fix for `tre` issue #8 plus `View.set_stylesheet`, and M92's animatable icon color. Unblocks M31.
...
- Step 1: pin `.venv` (Phase 1 Step 3): build a `tre` v0.3.3 wheel offline from the `v0.3.3` tag (the checkout M32's sizing already built from) and install it into `.venv` in place of the `tre.pth` source-tree link; confirm `tre` then resolves to the installed wheel, not `tre`'s source tree. Record in `installation.md` how to point `.venv` back at `tre`'s source tree for deliberate testing — ⬜
- Step 2: run `tools/migrate_views_0_3_3.py` over `src/` and `examples/`, and over the 8 `.py` files with embedded YAML; review every diff (comments preserved) and commit it on its own so the mechanical change is reviewable apart from the hand edits — ⬜
- Step 3: hand-migrate what the script can't reach: Python dict specs in the tests, and the fragments' `params:`/comments that name old fields (e.g. `Switch_Component.yaml`'s header, per Phase 1 Step 2) — ⬜
