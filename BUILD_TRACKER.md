...
| M30 — Theme Arguments on `App` and `App.load()` | `███░░░░░░░` 33% | 🚧 In progress — Phase 1 of 3 done (2026-09-25) |
| M31 — Hot Reload for Theme and Stylesheet Files | `░░░░░░░░░░` 0% | ⬜ Not started — waiting on tre issue #8 |

**Just closed:** M30 Phase 1 (2026-09-25) — the user chose the theme API: an app-wide theme on `App(...)`, a per-screen stylesheet on `App.load()` with an app-wide default, and theme-file hot reload deferred to a new M31 until `tre` issue #8 is fixed. Before that, M29 — Tesserae owns all file handling and gives `tre` no file path anywhere: views go over as one finished dict (with `include:` resolved in Tesserae), images as RGBA pixels (Pillow), themes/stylesheets as `*_spec=` dicts and fonts as bytes (`tre` M86), and `app.run(hot_reload=True)` reloads a running app from `watchfiles` events via `tre` M87's `App.thread_handle()` (requested as `tre` issue #6). Six user-approved phases, each checked against `tre`'s real source first and mutation-checked; 157 → 221 tests.
...
**Up next:** M30 Phase 2 — implement the API the user chose on 2026-09-25: an app-wide theme on `App(...)`, and a stylesheet per screen on `App.load()` with an app-wide default on `App(...)`; waiting on the user's go-ahead to start. Then M31 (hot reload for theme and stylesheet files), deliberately deferred by the user until [`tre` issue #8](https://github.com/mindderivative/tre/issues/8) is fixed. Other named, un-scoped candidates: hot reload for `App.register()`ed screens, and conditional per-item styling for the 5 Rust-internal-state-dependent-coloring widgets (`tabs`/`button_group`/`navigation_rail`/`navigation_drawer`/`menu`).
...
**Status: 🚧 In progress — Phase 1 of 3 done (2026-09-25).** Scoped 2026-09-25; User-directed: "scope adding theme args to App.load()". Closes the long-standing known gap that `App` has no theme API at all. Since M29, `load_view` already accepts every theme argument (`theme_seed`/`dark`/`default_theme`/`custom_theme`/`stylesheet`, reading the files in Tesserae and handing `tre` `*_spec=` dicts), so the plumbing exists — the real work is deciding where a theme *lives* in a multi-screen `App`, which `tre` itself constrains.
...
- Step 1: `App.__init__` gains `theme_seed=`, `dark=`, `default_theme=`/`default_theme_spec=`, `custom_theme=`/`custom_theme_spec=` and the default `stylesheet=`/`stylesheet_spec=`. Theme and stylesheet files are read once, through `load_theme`/`load_stylesheet` (so a `FontFallbackWarning` fires once, not once per screen), and kept as dicts — `tre` still never gets a path. Giving a path and its `*_spec=` twin together is an error, as in `load_view` — ⬜
- Step 2: `App.load(view_path, viewmodel_cls, name=None, *, stylesheet=None, stylesheet_spec=None)` builds the screen through `load_view` with the app's theme dicts and either its own stylesheet or the app's default — ⬜
- Step 3: `App.register()` is unchanged (the caller builds and themes the view); its docstring says how to give a registered screen the app's theme and stylesheet — ⬜
- Step 4: drop `App.load()`'s docstring note that it takes no theme arguments — ⬜
...
**Status: ⬜ Not started — waiting on tre issue #8.** Split out of M30 on 2026-09-25: the user chose to wait until [`tre` issue #8](https://github.com/mindderivative/tre/issues/8) is fixed rather than ship theme-file hot reload now. Today `View.set_theme` makes every node with a `{{ }}` binding show its static YAML value until its `Signal` next changes (reproduced against `tre` v0.3.2), so re-theming a running app on every theme-file save would visibly revert live data; and `tre` has no `View.set_stylesheet`, so stylesheet files can't be re-applied at all. Builds on M30's app-wide theme and per-screen stylesheets.

### Phase 1 — Theme Files ⬜
- Step 1: once `tre` issue #8 part 1 is fixed (bindings re-applied after `set_theme`), with `run(hot_reload=True)` also watch the app's theme files; on change, re-read them on the watcher thread and queue one `View.set_theme(...)` per `load()`ed screen via `call_soon`, passing the complete selection each time (seed, dark, both theme dicts), since `tre` treats omitted arguments as defaults — ⬜
...
- Step 1: once `tre` issue #8 part 2 is fixed (`View.set_stylesheet`), watch the app's default stylesheet and each screen's own, re-applying them the same way — ⬜
