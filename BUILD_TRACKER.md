...
**Up next:** M32 Phase 5 — close the known gaps 0.3.3 fixes (the 0.3.3 migration gap, and M29's bound-value gap via `tre` M91) into "Fixed gaps", update the stylesheet gap (`View.set_stylesheet` now exists), mark M31 unblocked, and a final docs sweep including the cascade note; waiting on the user's go-ahead. Then M31.
...
**Status: 🚧 In progress — Phase 4 of 5 done (2026-09-25).** User-directed: "scope the 0.3.3 migration as M32"; decisions: "go with your recommendations for M32". `tre` v0.3.3 is released (tag on `bc5e9b6` on `tre`'s `main`, 24 release assets; verified), carrying M90's schema and API renames (full table in `tre`'s `docs/migrating-0.3.3.md`), M91's fix for `tre` issue #8 plus `View.set_stylesheet`, and M92's animatable icon color. Unblocks M31.
...
- Step 1: MkDocs, per the standing rule — every YAML and Python snippet in `docs/` updated to 0.3.3 names; a note on the cascade change (stylesheet layout rules now apply); `installation.md` names `tre` 0.3.3; README/ARCHITECTURE snippets; `mkdocs build --strict` clean — ⬜
- Step 2: tracker — close the 0.3.3 gap and M29's bound-value gap (fixed by `tre` M91) into "Fixed gaps", update the stylesheet gap (`View.set_stylesheet` now exists), mark M31 unblocked; republish the artifact; `PLAN.md`/`LOG.md` — ⬜
