...
`pytest tests/` 214 passed (+18). Phases 1–3, the exact-commit CI pin and the `tre` issue #6 link are pushed; Phase 4 committed locally, not pushed.

**Up next:** Phase 5 — a final docs pass (the user-facing rule "give Tesserae paths; Tesserae never gives `tre` one" stated once, clearly, plus `ARCHITECTURE.md`'s layering). Much of it already landed phase by phase. Hot reload inside `App.run()` waits on [`tre` issue #6](https://github.com/mindderivative/tre/issues/6).
