...
Then install `tre` v0.3.3, either from the wheel for your platform on the
[v0.3.3 release page](https://github.com/mindderivative/tre/releases/tag/v0.3.3):

```bash
pip install /path/to/downloaded/tre-0.3.3-<your-platform>.whl
```

or by building that tag from source:

```bash
git clone --branch v0.3.3 https://github.com/mindderivative/tre.git ../tre-v0.3.3
pip install maturin
python -m maturin build --release --manifest-path ../tre-v0.3.3/crates/engine-py/Cargo.toml --out dist
pip install dist/tre-0.3.3-*.whl
```
...
!!! note "Testing against unreleased `tre`"
    ...
    v0.3.3 wheel (`pip install --force-reinstall ...`) before trusting a
    test run.
