...
| `tesserae.overlays` | MD3's overlays on `tre`'s layers (M41): `Dialog`, `Menu` (anchored, at a point, or a context menu), `Snackbar`, `Tooltip`, `SideSheet`, `NavigationDrawer`, `SearchView` (M42); each has `open()`, `close()`, `on_close(fn)`. See [Overlays](../guide/overlays.md). |
